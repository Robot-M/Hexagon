...
namespace Stone.Core  
{
	...
	public struct Hex
	{
	    public Hex(int q=0, int r=0, int s=0)
	    {
	        this.q = q;
	        this.r = r;
	        this.s = s;
		...
		{
			Hex hex = (Hex)b;
			return this.q == hex.q && this.s == hex.s && this.r == hex.r;
		}
		...
		static public Hex operator*(int k, Hex a)
		{
			return new Hex(a.q * k, a.r * k, a.s * k);
		}
		...
		static public int DirectionIndex(Hex direct)
		{
			int directIndex = directions.FindIndex(delegate(Hex o){
				return o == direct;
			});
			return directIndex;
		}
		...
		{
			return Hex.DiagonalNeighbor (this, direction);
		}
		...
		static public List<Hex> Ring(Hex center, int radius)
		{
			...
			if (radius == 0) {
				...
				Hex start = Hex.Add (center, Hex.Scale (directions [4], radius));
				for (int i = 0; i < 6; i++) {
					for (int j = 0; j < radius; j++) {
						results.Add (start);
						start = Hex.Neighbor (start, i);
					}
				}
			}
			return results;
		...
		/// <summary>
		///  获取两点之间的连线。区别于使用<线性插值法> FractionalHex.HexLinedraw
		/// </summary>
		/// <returns>The linedraw.</returns>
		/// <param name="origin">Origin.</param>
		/// <param name="target">Target.</param>
		static public List<Hex> HexLinedraw(Hex origin, Hex target)
		{
			List<int> paths = Hex.DirectionPath (origin, target);
			List<Hex> results = new List<Hex> ();
			for (int i = 0; i < paths.Count; i++) {
				if (i == 0) {
					results [i] = Hex.Add (directions [paths [i]], origin);
				} else {
					results [i] = Hex.Add (directions [paths [i]], results [i-1]);
				}
			}
		...
		/// <param name="origin">Origin.</param>
		/// <param name="target">Target.</param>
		static public List<int> DirectionPath(Hex origin, Hex target)
		{
			List<int> results = new List<int> ();
			Hex sub = Hex.Subtract (target, origin);
			int len = Hex.Length (sub);

			int maxType = 1;
			if (Math.Abs (sub.q) == len) {
				maxType = 1;
			} else if (Math.Abs (sub.r) == len) {
				maxType = 2;
			} else {
				maxType = 3;
			}
			...
			Hex direct1;
			if (maxType < 3) {
				int q = sub.q == 0 ? 0 : sub.q / Math.Abs (sub.q);
				int r = sub.r == 0 ? 0 : sub.r / Math.Abs (sub.r);
				direct1 = new Hex (q, r, -q - r);
			} else {
				int r = sub.q == 0 ? 0 : sub.q / Math.Abs (sub.q);
				int s = sub.s == 0 ? 0 : sub.s / Math.Abs (sub.s);
				direct1 = new Hex (-r-s, r, s);
			}

			int directIndex = Hex.DirectionIndex (direct1);
			for(int i=0; i<len; i++){
				results.Add (directIndex);
			}

			// 第二步：计算第二个值需要走过的步数，并合并到结果中
			Hex direct2;
			int resLen;
			if (maxType == 1) {
				resLen = sub.r - direct1.r * len;
				int r = resLen / Math.Abs (resLen);
				direct2 = new Hex (0, r, -r);
			}else if (maxType == 2) {
				resLen = sub.q - direct1.q * len;
				int q = resLen / Math.Abs (resLen);
				direct2 = new Hex (q, 0, -q);
			} else {
				resLen = sub.r - direct1.r * len;
				int r = resLen / Math.Abs (resLen);
				direct2 = new Hex (-r, r, 0);
			}

			resLen = Math.Abs (resLen);
			if(resLen != 0){
				direct2 = Hex.Add (direct1, direct2);
				int directIndex2 = directions.FindIndex(delegate(Hex o){
					return Hex.Equals(o, direct2);
				});
				for(int i=0; i<len; i++){
					results[i] = directIndex2;
				}
			}
	...
	public struct Layout
	{
	    ...
	    static public FractionalHex PixelToFHex(Layout layout, Point p)
	    {
	        Orientation M = layout.orientation;
	        Point size = layout.size;
	        Point origin = layout.origin;
		...
		static public Hex PixelToHex(Layout layout, Point p)
		{
			return FractionalHex.HexRound(Layout.PixelToFHex(layout, p));
		}
	...
	public struct Tests
	{
	    ...
	    static public void TestLayout()
	    {
	        ...
	        Tests.EqualHex("layout", h, Layout.PixelToHex(flat, Layout.HexToPixel(flat, h)));
	        Layout pointy = new Layout(Layout.pointy, new Point(10, 15), new Point(35, 71));
	        Tests.EqualHex("layout", h, Layout.PixelToHex(pointy, Layout.HexToPixel(pointy, h)));
	    }
